        ...
        builder: (context,snapshot){
          if(snapshot.hasData){
            return new ListView.builder(
              itemBuilder: (BuildContext context, int index) => new PostItem(snapshot.data.posts[index]), // building listails from data
  ...
  Widget build(BuildContext context) {
    Widget _buildTiles(Post root){
      return new ExpansionTile(
          title: new Row(
            children: <Widget>[
              post.thumbnailUrl != null ? // check if there is a image
                  new ClipOval(                 //creating oval cliph for the thumbnail
                    child: new Image.network( //retrieving the image from the network
                      post.thumbnailUrl,
                      height: 50.0,
                      fit: BoxFit.fill
                    ),
          ...
          children: <Widget>[
            ...
            new ListTile(
              ...
              onTap:(){
                Navigator.push(
                  context,
                  new MaterialPageRoute(builder: (context) => new SingleArticle(post)),
                );
              } ,
            )
          ]
...
class SingleArticle extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    Widget cover = post.thumbnailUrl != null ? new Image.network(
        post.medThumb,
        height: 200.0,
        fit: BoxFit.cover
    ) : new Column();
  ...
  Widget build(BuildContext context) {
    ...
    Widget response = new FutureBuilder<FLResponse>(
        ...
        builder: (context,snapshot){
          if(snapshot.hasData){
            return new ListView.builder(
              itemBuilder: (BuildContext context, int index) => new EventPostItem(snapshot.data.eventposts[index]),
                ...
                child: new Column(
                  children: <Widget>[
                    ...
                    new ListTile(
                      ...
                      trailing: new Icon(Icons.arrow_forward, color: Colors.deepOrange,),
                      onTap:(){
                        Navigator.push(
                          context,
                          new MaterialPageRoute(builder: (context) => new SingleArticle(post)),
